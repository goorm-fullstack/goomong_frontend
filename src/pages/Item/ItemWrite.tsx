...
export default function ItemWrite() {
  ...
  const handleSumbit = () => {
    ...
    if (imgRef.current && imgRef.current.files) Object.values(imgRef.current.files).forEach((file) => data.append('multipartFiles', file));
  ...
  return (
    <>
      <h1>아이템 입력</h1>
      <input accept="image/*" ref={imgRef} type="file" multiple></input>
